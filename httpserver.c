...
void serve_file(int fd, char* path) {
  ...
  char intbuf[100];
  sprintf(intbuf, "%d", size);

  
  int bytes_read = read(i, buf, size);

  http_start_response(fd, 200);
  http_send_header(fd, "Content-Type", http_get_mime_type(path));
  http_send_header(fd, "Content-Length", intbuf); // TODO: change this line too
  http_end_headers(fd);
...
void serve_directory(int fd, char* path) {
  ...
  while ((entry = readdir(dir)) != NULL) {
    if (strcmp(http_get_mime_type(entry->d_name), "text/html") == 0) {
      set = 1;
      break;
    }
  }
  if (set) {
    ...
    while ((entry = readdir(dir)) != NULL) {
      char href[1024];
      http_format_href(href, path, entry->d_name);
      dprintf(fd, "%s\n", href);
    }
  }
  /* PART 3 END */
...
void handle_proxy_request(int fd) {
  ...
  */
  ...
  struct args* client_to_poxy = malloc(sizeof(struct args));
  struct args* poxy_to_client = malloc(sizeof(struct args));
  client_to_poxy->source = fd;
  client_to_poxy->dest = target_fd;
  client_to_poxy->ret = 0;
  poxy_to_client->source = target_fd;
  poxy_to_client->dest = fd;
  poxy_to_client->ret = 0;
  ...
  while (1) {
    ...
    pthread_create(&p0, NULL, helper_method, (void*)client_to_poxy);
    pthread_create(&p1, NULL, helper_method, (void*)poxy_to_client);

    if (client_to_poxy->ret == 1 || poxy_to_client->ret == 1){
      close(fd);
      close(target_fd);
      free(client_to_poxy);
      free(poxy_to_client);
...
void* helper_method(void* a){
  struct args* b = (void*)a;
  int source = b->source;
  int dest = b->dest;
  char buf[100000];
  int r = read(source, buf, 100000);
  if (r <= 0){
    b->ret = 1;
  } else {
    write(dest, buf, r);
...
void init_thread_pool(int num_threads, void (*request_handler)(int)) {
  ...
  for (int i = 0; i < num_threads; i++) {
    pthread_t p;
    pthread_create(&p, NULL, handle_clients, (void*)request_handler);
  }
  /* PART 7 END */
}
#endif
...
void serve_forever(int* socket_number, void (*request_handler)(int)) {
  ...
  *socket_number = socket(PF_INET, SOCK_STREAM, 0);
  ...
  while (1) {
    ...
    pthread_t p;
    pthread_create(&p, NULL, request_handler, (void*)client_socket_number);
    /* PART 6 END */
#elif POOLSERVER
    /*
     * TODO: PART 7
